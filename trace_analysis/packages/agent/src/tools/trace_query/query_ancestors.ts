...
import { queryAncestors, queryById, TraceQuery, getTreeStyleTraceEvents } from '@lynx-js/trace-query';
...
export const queryAncestorsTool = tool(
  async ({ slice_id, index }, config) => {
    try {
      ...
      const result = await queryAncestors(traceQuery, slice_id);
  ...
  {
    name: 'query_ancestors',
    description:
      'Query all ancestor (parent) slices of a given slice. Returns the complete parent chain from the slice up to the root, organized as a tree structure. Useful for understanding the context and call hierarchy of a specific event.',
    schema: z.object({
      slice_id: z.number().describe('The ID of the slice to query ancestors for.'),
      index: z
        .number()
        .optional()
