...
export const prompt: Prompt = {
  name: 'timing-flag',
  description:
    'A reference guide for diagnosing issues related to Lynx Timing Flags. It analyzes trace events to troubleshoot missing performance callbacks, invalid timing flags, and abnormal metric values (like excessively long ActualFMP).',
  prompt: `# Lynx Timing Flag Analysis Guide
...
* **Native Injection**: Injected via \`updateData\` or container timestamps via \`setExtraTiming\` (crucial for \`totalFcp\` and \`totalActualFmp\`).

### 3. Lynx >= 3.7 PerformanceEntry Migration (Callback "not firing")

In Lynx 3.7+, several \`PerformanceEntry\` types are deprecated and replaced by \`PipelineEntry\` and \`LoadBundleEntry\`. If a user's performance callback suddenly stops firing after upgrading, this is frequently the root cause.
...
* **Replacement (Lynx 3.7+)**:
  * \`LoadBundleEntry\` and \`PipelineEntry\`, both delivered as \`entryType === "pipeline"\`

Practical mapping for front-end migration:

* Old \`metric.fcp\` callback -> use \`LoadBundleEntry\` (\`name === "loadBundle"\`), read \`fcp\` / \`lynxFcp\` / \`totalFcp\`.
* Old \`metric.actualFmp\` callback -> use \`PipelineEntry\` (filter by \`identifier\`), read \`actualFmp\` / \`lynxActualFmp\` / \`totalActualFmp\`.
* Old \`Init*\` callbacks -> use timestamps embedded in \`LoadBundleEntry\` (e.g., \`openTime\`, \`prepareTemplateStart\`, \`containerInitStart\`).

Minimal observer pattern (recommended):
...
#### Scenario 0: Performance callback never fires (or stops after upgrading)

* **Symptom**: The user reports that the \`PerformanceObserver\`/\`createObserver\` callback is never invoked, especially after upgrading to Lynx >= 3.7.
* **Investigation Action**:
  * First, inspect the observer registration target(s): are they still using \`metric.fcp\`, \`metric.actualFmp\`, or any \`Init*Entry\`?
  * As a fast validation, temporarily switch to \`observer.observe(["pipeline"])\` and log every received entry (or stringify it).
* **Verdict Logic**:
  * If \`entryType === "pipeline"\` entries are received, but the original callback never matched (or never fired), the code is likely observing/filtering deprecated entries.
  * If no entries are received at all, fallback to the "Late Registration" diagnosis in Scenario 1/2.
* **Recommendation**: Migrate to \`LoadBundleEntry\`/\`PipelineEntry\` as described in "Core Knowledge -> Lynx >= 3.7 PerformanceEntry Migration".
...
* **Verdict Logic**:
  ...
  * If the user is on Lynx >= 3.7 and is still listening to deprecated entries -> Apply Scenario 0 first.
...
* **Verdict Logic 2**:
  * If the event is MISSING -> **Late Registration**. The listener was not ready when the pipeline finished.
  * If the user is on Lynx >= 3.7 and is still listening to \`metric.actualFmp\` (or any deprecated entry) -> Apply Scenario 0 first.
...
#### Scenario 4: \`actualFmp\` / \`lynxActualFmp\` is Abnormally Large (e.g., > 10 seconds)

* **Symptom**: The duration is unrealistically long.
...
* **Symptom**: The duration is unrealistically long.
* **Investigation Action**: Check \`openTime\` (container timestamp in \`LoadBundleEntry\`, or legacy \`InitContainerEntry\`); check if the page was preloaded.
* **Verdict Logic**:
  * If \`openTime\` is 0 or missing -> **Missing Container Timestamp**. Native client failed to call \`setExtraTiming\` before \`renderTemplate\`.
...
2. **Lynx >= 3.7: Prefer Pipeline Entries**: Avoid \`metric.*\` and legacy \`Init*Entry\` observers; use \`observer.observe(['pipeline'])\` and filter by \`entry.name\` / \`entry.identifier\`.
3. **Long List Anti-Pattern**: If tracking a list, place ONE flag on the \`<list>\` component itself. If you must flag items, use the **SAME flag name** for all items. Using unique names per item causes a callback storm and severe performance degradation.
4. **Meaningful Naming**: Use \`__lynx_timing_actual_fmp\` specifically for the element that indicates the page is ready for the user.`,
};
