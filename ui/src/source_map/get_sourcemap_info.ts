...
export async function getSourceFileInfo(
  ...
  try {
    ...
    if (sourceMap !== undefined) {
      ...
      if (!consumer) {
        const newConsumer = await new sourcemap.SourceMapConsumer(
          JSON.parse(sourceMap.data),
        );
        sourceMapState.edit((draft) => {
          draft.sourceMapConsumerByUrl.set(url, newConsumer);
        });
    ...
    console.warn(`getSourceFileInfo failed: ${String(error)}`);
    return null;
  }
}
