 ...
 */
export class UITreeView extends Component<UITreeViewProps, UITreeViewState> {
  /**
  ...
  constructor(props: UITreeViewProps) {
    ...
    if (props.selectedUI && props.rootUI) {
      this.treeData = this.constructTreeData(props.rootUI, props.selectedUI);
    }
    this.state = {
   ...
   */
  constructTreeData = (rootUI: LynxUI, selectedUI: LynxUI) => {
    const constructTreeDataRecursively = (
      ...
      this.keysToUITreeDataNodeMap.set(current.id, currentTreeNode);
  ...
  handleKeyDown(event: React.KeyboardEvent) {
    ...
    if (handler != null) {
      handler.call(
        this,
        this.keysToUITreeDataNodeMap.get(this.state.selectedKeys[0]),
      );
  ...
  expandCurrentNode = (current: UITreeDataNode | undefined) => {
    if (!current || this.state.expandedKeys.includes(current.key)) {
      return;
    }
    if (current.children.length <= 0) {
      return;
