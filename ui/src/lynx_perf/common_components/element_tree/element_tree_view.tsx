...
> {
  ...
  handleKeyDown(event: React.KeyboardEvent) {
    const handlers: Record<
      string,
      (node: ElementTreeDataNode | undefined) => void
    > = {
    ...
    if (handler != null) {
      handler.call(
        this,
        this.keysToElementTreeDataNodeMap.get(this.state.selectedKeys[0]),
      );
  ...
  /**
   * Handles expanded node changes from the tree component.
   */
  ...
  collapseCurrentNode = (current: ElementTreeDataNode | undefined) => {
    ...
    const removedExpandedKeys: string[] = [];
    const traverse = (node: ElementTreeDataNode) => {
      if (node.children.length > 0) {
        node.children.forEach((child: ElementTreeDataNode) => {
          traverse(child);
        });
        removedExpandedKeys.push(node.key);
      }
    };
    traverse(current);
    ...
    this.setState({
      expandedKeys: this.state.expandedKeys.filter(
        (key) => !removedExpandedKeys.includes(key),
      ),
    });
  };
}
