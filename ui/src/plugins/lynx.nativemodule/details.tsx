...
export class NativeModuleDetailsPanel implements TrackEventDetailsPanel {
  ...
  async load({eventId}: TrackEventSelection) {
    ...
    const selectionDetail = await this.ctx.tracks
      ...
      ?.renderer.getSelectionDetails?.(eventId);
    if (this.sliceDetail && selectionDetail) {
      this.sliceDetail.name =
        lynxPerfGlobals.state.traceIdToJSBName.get(eventId) || '';
  ...
  private async assembleOptimizedSections(flows: Flow[]) {
    const sections: NativeModuleSection[] = [];
    const nativeModuleInvoke = this.findSectionTs(NATIVEMODULE_INVOKE, flows);
    ...
    if (
      nativeModuleInvoke === undefined ||
      platformCallbackStart === undefined ||
      nativeModuleCallback === undefined
    ) {
      return sections;
    }
    const jsValueToPubValue = await this.getDescendantsWithSpecificName(
      'JSValueToPubValue',
      nativeModuleInvoke.sliceId,
    );
    const pubValueToJSValue = await this.getDescendantsWithSpecificName(
      'PubValueToJSValue',
    ...
    if (
      ...
      callPlatformImplementation === undefined
    ) {
      return sections;
    }
    if (isSpecialNativeModule(flows)) {
      ...
      threadInfo[nativeModuleInvoke.threadName] =
        callPlatformImplementation.dur;
      threadInfo['other'] =
        Number(platformCallbackStart.sliceStartTs) -
        callPlatformImplementation.dur -
        callPlatformImplementation.ts;
      sections.push({
        beginTs: jsValueToPubValue.ts + jsValueToPubValue.dur,
  ...
  private async getDescendantsWithSpecificName(
    traceName: string,
    traceId: number,
  ) {
    const query = `
  ...
  render() {
    ...
    const hasSections =
      this.nativeModuleSections && this.nativeModuleSections.length > 0;
    if (this.sliceDetail) {
      return m(
        DetailsShell,
        {
          ...
          buttons: m(Button, {
            ...
            onclick: (_e) => {
              this.ctx.selection.selectSqlEvent(
                'slice',
        ...
        hasSections &&
          m(
            ...
            m(
              ...
              this.renderRhs(this.ctx, this.sliceDetail),
        ...
        !hasSections &&
          m(GridLayout, this.renderRhs(this.ctx, this.sliceDetail)),
      );
    } else {
      return null;
    }
  ...
  private renderRhs(trace: Trace, slice: SliceDetails): m.Children {
    ...
    const details =
      this.sliceDetail &&
      m(Tree, [
        m(TreeNode, {left: 'Name', right: this.sliceDetail.name}),
        m(TreeNode, {
          left: 'Category',
          right:
        ...
        m(TreeNode, {
          left: 'Start time',
          right: m(Timestamp, {trace, ts: this.sliceDetail.ts}),
        }),
        m(TreeNode, {
          left: 'Duration',
          right: m(DurationWidget, {trace, dur: this.sliceDetail.dur}),
        }),
      ]);
    const detailSection =
      this.sliceDetail && m(Section, {title: 'Details'}, details);
