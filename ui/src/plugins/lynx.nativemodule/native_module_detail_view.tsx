    ...
    const stagDetailColumns = [
      {
        ...
        render: (
          value: string,
          record: (typeof stagDetailDataSource)[0],
          index: number,
        ) => (
          <div style={{display: 'flex', alignItems: 'center'}}>
            ...
            {this.awatingCallbackStage(record.thread, index) && (
              <img
                src={AWATIMING_CALLBACK_STAGE_DOTTED_IMAGE}
                style={{width: 10, height: 15, marginRight: 5, flexShrink: 0}}
              />
            )}
            <div>{value}</div>
          </div>
