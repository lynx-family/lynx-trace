...
export default class LynxThreadGroupPlugin implements PerfettoPlugin {
  ...
  async onTraceLoad(ctx: Trace): Promise<void> {
    const children = ctx.currentWorkspace.children.slice();
    ctx.currentWorkspace.clear();
    // Filter out process groups
    const processGroup = children.filter((child: TrackNode) =>
      child.uri?.startsWith('/process'),
    );
    processGroup.forEach((group: TrackNode) => {
      const pids = group.name.split(' ');
      ...
      const sortedThread = threadGroup
        .filter((thread: TrackNode) => thread.uri?.startsWith('/thread'))
        .sort((a: TrackNode, b: TrackNode) => {
          const aTids = a.name.split(' ');
          const aTid = parseInt(aTids[aTids.length - 1]);
          const bTids = b.name.split(' ');
      ...
      threadGroup.forEach((thread: TrackNode) => {
        group.addChildInOrder(thread);
      });
    });
    children.forEach((child: TrackNode) => {
      ctx.currentWorkspace.addChildInOrder(child);
    });
  }
