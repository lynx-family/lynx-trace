...
export default class LynxPerf implements PerfettoPlugin {
  ...
  async onTraceLoad(ctx: Trace): Promise<void> {
    ...
    ctx.tracks.registerTrack({
      ...
      renderer: new LynxPerfTrack(ctx, LYNX_ISSUES_PLUGIN_ID),
    });
    ...
    ctx.commands.registerCommand({
      ...
      callback: () => {
        ...
        for (const item of workspace.children) {
          ...
          if (
            item.children.some((child) => child.uri === LYNX_ISSUES_PLUGIN_ID)
          ) {
            break;
          }
          ...
          if (
            ...
            const track = new TrackNode({
              name: `${TRACK_TITLE} (count: ${lynxPerfGlobals.state.issues.length})`,
              uri: LYNX_ISSUES_PLUGIN_ID,
