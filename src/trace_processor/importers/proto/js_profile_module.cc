  ...
  return reader.ForEachField([&](std::string_view key) -> json::FieldResult {
    if (key == "functionName") {
      if (auto s = reader.GetString()) {
        call_frame.function_name = std::string(*s);
        if (call_frame.function_name.empty()) {
          call_frame.function_name = "(anonymous)";
        }
      }
      return json::FieldResult::Handled{};
    }
    if (key == "columnNumber") {
      call_frame.column_number = static_cast<int32_t>(
          reader.GetInt64().value_or(call_frame.column_number));
      return json::FieldResult::Handled{};
    }
    if (key == "lineNumber") {
      call_frame.line_number = static_cast<int32_t>(
          reader.GetInt64().value_or(call_frame.line_number));
      return json::FieldResult::Handled{};
    }
    if (key == "url") {
      if (auto s = reader.GetString()) {
        call_frame.url = std::string(*s);
      }
      return json::FieldResult::Handled{};
    }
...
base::Status ParseNode(json::SimpleJsonParser& reader,
  ...
  RETURN_IF_ERROR(
      reader.ForEachField([&](std::string_view key) -> json::FieldResult {
        if (key == "id") {
          profile_node.id = static_cast<int32_t>(reader.GetInt64().value_or(0));
          return json::FieldResult::Handled{};
        }
        if (key == "parent") {
          profile_node.parent =
              static_cast<int32_t>(reader.GetInt64().value_or(-1));
          return json::FieldResult::Handled{};
        }
        if (key == "children" && reader.IsArray()) {
          RETURN_IF_ERROR(reader.ForEachArrayElement([&]() {
            if (auto child = reader.GetInt64()) {
              profile_node.children.push_back(static_cast<int32_t>(*child));
            }
        ...
        if (key == "callFrame" && reader.IsObject()) {
          RETURN_IF_ERROR(ParseCallFrame(reader, profile_node.call_frame));
          return json::FieldResult::Handled{};
        }
        return json::FieldResult::Skip{};
      }));
...
base::Status ParseNodes(json::SimpleJsonParser& reader,
  ...
  RETURN_IF_ERROR(reader.ForEachArrayElement([&]() {
    ...
    return ParseNode(reader, cpu_profile, node_map);
  }));
  if (cpu_profile.nodes.empty()) {
    return base::ErrStatus("JS profile nodes array is empty");
  }
  return base::OkStatus();
}
...
base::Status ParseProfile(json::SimpleJsonParser& reader,
  ...
  RETURN_IF_ERROR(
      reader.ForEachField([&](std::string_view key) -> json::FieldResult {
        if (key == "startTime") {
          cpu_profile.start_timestamp = reader.GetInt64().value_or(0);
          return json::FieldResult::Handled{};
        }
        if (key == "endTime") {
          cpu_profile.end_timestamp = reader.GetInt64().value_or(0);
          return json::FieldResult::Handled{};
        }
        if (key == "samples" && reader.IsArray()) {
          RETURN_IF_ERROR(reader.ForEachArrayElement([&]() {
            if (auto sample = reader.GetInt64()) {
              cpu_profile.samples.push_back(static_cast<int32_t>(*sample));
            }
            return base::OkStatus();
          }));
          return json::FieldResult::Handled{};
        ...
        if (key == "timeDeltas" && reader.IsArray()) {
          ...
          if (result.ok()) {
            cpu_profile.time_deltas = std::move(*result);
          }
          return json::FieldResult::Handled{};
        }
        if (key == "nodes" && reader.IsArray()) {
          RETURN_IF_ERROR(ParseNodes(reader, cpu_profile, node_map));
          return json::FieldResult::Handled{};
        }
        return json::FieldResult::Skip{};
      }));
  if (cpu_profile.time_deltas.size() != cpu_profile.samples.size()) {
    return base::ErrStatus("samples size is not equal time_deltas size");
  }
  return base::OkStatus();
}
...
base::Status ParseProfileJson(
  ...
  return reader.ForEachField([&](std::string_view key) -> json::FieldResult {
    if (key == "profile" && reader.IsObject()) {
      RETURN_IF_ERROR(ParseProfile(reader, cpu_profile, node_map));
      return json::FieldResult::Handled{};
    }
    return json::FieldResult::Skip{};
  });
        ...
        if (start_node != node_map.end() && end_node != node_map.end()) {
          ...
          std::unique_ptr<TrackEventData> begin_data(
              new TrackEventData(TraceBlobView(TraceBlob::CopyFrom(
                                 ...
                                 state));
          module_context->track_event_stream->Push(begin_timestamp,
                                                   std::move(*begin_data));
          ...
          module_context->track_event_stream->Push(end_timestamp,
                                                   std::move(*end_data));
...
JSProfileModule::JSProfileModule(ProtoImporterModuleContext* module_context,
                                 TraceProcessorContext* context)
    : ProtoImporterModule(module_context), context_(context) {
  RegisterForField(TracePacket::kJsProfilePacketFieldNumber);
...
ModuleResult JSProfileModule::DecodeJsProfilePacket(
  ...
  if (!status.ok()) {
    ...
    PERFETTO_ELOG("TokenizeJsProfilePacket::json_parser_failure: %s",
  ...
  // Emit trace events
  EmitProfileEventsToTrace(events, node_map, cpu_profile, state,
                           module_context_);
